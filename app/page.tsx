...
const theme = {
  ...
  color: "#ffc2d1",
  bg: "bg-[#fff0f3]",
  border: "border-[#ffb3c6]",
  primaryText: "text-[#fb6f92]",
  primaryBg: "bg-[#fb6f92]",
  secondaryBg: "bg-[#ff8fab]",
  secondaryText: "text-[#ff8fab]",
...
export default function Home() {
  ...
  useEffect(() => {
    const targetDate = new Date("2026-10-15T19:00:00").getTime();
  ...
  const openEnvelope = () => {
    ...
    if (audio1Ref.current) {
      audio1Ref.current.play().then(() => setIsPlaying(true)).catch(e => {
  ...
  const handleTimeUpdate = () => {
    if (audio1Ref.current && audio1Ref.current.currentTime >= 14 && activeTrack === 'track1') {
      audio1Ref.current.pause();
      setActiveTrack('track2');
      if (isPlaying && audio2Ref.current) {
        audio2Ref.current.play().catch(e => console.log("Audio 2 play failed:", e));
      }
    }
  };
  ...
  return (
    <main className={`min-h-screen flex flex-col items-center justify-start font-[family-name:var(--font-cairo)] relative overflow-x-hidden ${theme.bg}`}>
      {/* Audio Elements */}
      <audio ref={audio1Ref} src="/music1.mp3" onTimeUpdate={handleTimeUpdate} />
      <audio ref={audio2Ref} loop src="/music2.mp3" />
      ...
      {isOpen && (
        <motion.button 
          ...
          onClick={toggleMusic}
          className={`fixed bottom-6 right-6 z-[999] w-14 h-14 ${theme.primaryBg} rounded-full shadow-2xl flex items-center justify-center text-white hover:scale-110 transition-all duration-500`}
        >
      ...
      <div className={`relative w-[90vw] max-w-[500px] h-[60vw] max-h-[350px] cursor-pointer perspective-[1000px] z-[200] transition-all duration-1000 ease-in-out mt-[15vh] ${isOpen ? 'opacity-0 pointer-events-none -translate-y-[50vh] fixed' : ''}`} onClick={openEnvelope}>
        ...
        <div className={`absolute top-0 left-0 right-0 h-[60%] ${theme.envDark} origin-top z-50`} style={{ clipPath: 'polygon(0 0, 50% 100%, 100% 0)' }}>
          <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[65px] h-[65px] ${theme.primaryBg} rounded-full text-white flex justify-center items-center font-[family-name:var(--font-cairo)] font-bold text-2xl shadow-[0_4px_10px_rgba(0,0,0,0.2)] z-[60]`}>هبة</div>
        </div>
        <div className={`absolute -bottom-[50px] left-1/2 -translate-x-1/2 font-bold ${theme.accentText} bg-white px-8 py-3 rounded-full shadow-[0_4px_10px_rgba(0,0,0,0.1)] animate-bounce whitespace-nowrap z-[70] text-xl`}>سبوع النونو 🍼</div>
      </div>
      ...
      <div className={`w-full ${theme.bg} transition-all duration-1000 delay-500 flex flex-col items-center absolute inset-x-0 top-0 ${isOpen ? 'opacity-100 min-h-screen pb-20' : 'opacity-0 h-screen overflow-hidden'}`}>
        ...
        <div className="w-full relative flex flex-col items-center justify-center p-5 pt-12 pb-24 text-center overflow-hidden bg-[url('https://www.transparenttextures.com/patterns/rice-paper-2.png')]">
          ...
          <motion.div initial="hidden" animate={isOpen ? "visible" : "hidden"} variants={fadeInUp} transition={{ delay: 0.9 }} className="flex flex-col items-center mt-8 mb-8 z-10 relative">
            ...
            <div className={`font-[family-name:var(--font-amiri)] ${theme.accentText} text-2xl sm:text-3xl leading-relaxed px-4 sm:px-10 max-w-3xl font-bold text-center`}>
              ﴿لِلَّهِ مُلْكُ السَّمَاوَاتِ وَالْأَرْضِ يَخْلُقُ مَا يَشَاء يَهَبُ لِمَنْ يَشَاء إِنَاثًا وَيَهَبُ لِمَن يَشَاء الذُّكُورَ * أَوْ يُزَوِّجُهُمْ ذُكْرَانًا وَإِنَاثًا وَيَجْعَلُ مَن يَشَاء عَقِيمًا إِنَّهُ عَلِيمٌ قَدِيرٌ﴾
            </div>
          </motion.div>
          ...
          {/* Families Section */}
          <motion.div initial="hidden" animate={isOpen ? "visible" : "hidden"} variants={fadeInUp} transition={{ delay: 1.3 }} className="flex flex-col items-center text-center px-4 max-w-3xl z-10 w-full mb-8">
            <p className={`text-2xl sm:text-3xl ${theme.accentText} mb-6 font-bold`}>بكل الحب والسرور، تتشرف</p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-10 w-full mb-10">
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <span className={`text-xl sm:text-2xl ${theme.accentBg} text-white px-8 py-2 rounded-full font-bold shadow-md whitespace-nowrap`}>الحاج / وليد مجدي فرحات</span>
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <span className={`text-xl sm:text-2xl ${theme.accentBg} text-white px-8 py-2 rounded-full font-bold shadow-md whitespace-nowrap`}>الأستاذ / ماجد محمد إمام</span>
              </div>
            </div>
            
            <p className={`text-2xl sm:text-3xl ${theme.accentText} mb-4 font-bold`}>بدعوتكم لمشاركتنا فرحة "سبوع" أميرتنا الجميلة</p>
          </motion.div>
          ...
          <motion.div initial="hidden" animate={isOpen ? "visible" : "hidden"} variants={fadeInUp} transition={{ delay: 1.9 }} className="flex flex-col items-center text-center px-4 max-w-3xl z-10 w-full mb-10">
            <p className={`text-2xl sm:text-3xl text-gray-700 mb-16 leading-relaxed max-w-2xl font-bold font-[family-name:var(--font-cairo)]`}>
              لتملأ حياتنا بهجةً وسروراً.. ولتكتمل فرحتنا بوجودكم.
            </p>
            ...
            <div className={`bg-white/80 p-8 sm:p-12 rounded-3xl shadow-xl border-2 ${theme.border} w-full max-w-2xl mb-16 flex flex-col gap-6 text-right backdrop-blur-md`}>
              <div className="flex items-center gap-4">
                ...
                <span className="text-xl sm:text-3xl font-bold text-[#4a4a4a] leading-relaxed">التاريخ: الخميس، ١٥ أكتوبر ٢٠٢٦ م</span>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-3xl sm:text-4xl">🕖</span>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeInUp} className={`w-full max-w-[800px] py-16 flex flex-col items-center text-center relative border-t ${theme.border} z-10`}>
            <h2 className={`font-[family-name:var(--font-amiri)] text-4xl sm:text-5xl ${theme.primaryText} mb-12 font-bold`}>باقي على السبوع</h2>
